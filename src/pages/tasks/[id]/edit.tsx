...
export default function EditTaskPage() {
  ...
  useEffect(() => {
    const loadTask = async () => {
      ...
      try {
        ...
        const response = await apiClient.casaGet(`tasks/${id}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await apiClient.casaPut(`tasks/${id}`, payload);
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="py-8">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h1 className="text-3xl font-light mb-2">Edit Task</h1>
                  <p className="text-indigo-100 text-lg">Update task details</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <main className="max-w-3xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
          ...
          <form onSubmit={handleSubmit} className="bg-white dark:bg-fintech-bg-secondary rounded-lg shadow">
            <div className="p-6 space-y-6">
              ...
              {/* Status */}
              <div>
                <label htmlFor="status" className="block text-sm font-medium text-gray-700 dark:text-fintech-text-primary">
                  Status
                </label>
                <select
                  id="status"
